...
namespace Utils.Structs.Parsers
{
    ...
    public class EnumParser<TEnum> : IStructParser<TEnum> where TEnum : struct
    {
        ...
        TEnum? IStructParser<TEnum>.Parse(string value) => EnumParser.Parse<TEnum>(value);

        TEnum IStructParser<TEnum>.ParseOrDefault(string value, TEnum @default) => EnumParser.ParseOrDefault(value, @default);
